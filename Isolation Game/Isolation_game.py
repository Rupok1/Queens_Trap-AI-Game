...
WIN = pygame.image.load("winn.png")
...
# -------human,computer initial position--------------
...
# -------------------check available squares for the current player----------------------------------------
...
# ---------------------check available squares for the opponent player to block them------------------------------------------------------
...
# ----------------------check if player 1 or player 2 has lost----------------------------------------------
def check_lose(currentRow, currentCol):
    if (currentRow == -1 or currentCol == -1):
        return False
    return not (
            (
                        -1 < currentRow - 1 and currentRow - 1 < BOARD_ROWS and -1 < currentCol - 1 and currentCol - 1 < BOARD_COLS and
                        board[currentRow - 1][currentCol - 1] == 0) or
            (-1 < currentRow - 1 and currentRow - 1 < BOARD_ROWS and -1 < currentCol and currentCol < BOARD_COLS and
             board[currentRow - 1][currentCol] == 0) or
            (
                        -1 < currentRow - 1 and currentRow - 1 < BOARD_ROWS and -1 < currentCol + 1 and currentCol + 1 < BOARD_COLS and
                        board[currentRow - 1][currentCol + 1] == 0) or
            (-1 < currentRow and currentRow < BOARD_ROWS and -1 < currentCol + 1 and currentCol + 1 < BOARD_COLS and
             board[currentRow][currentCol + 1] == 0) or
            (
                        -1 < currentRow + 1 and currentRow + 1 < BOARD_ROWS and -1 < currentCol + 1 and currentCol + 1 < BOARD_COLS and
                        board[currentRow + 1][currentCol + 1] == 0) or
            (-1 < currentRow + 1 and currentRow + 1 < BOARD_ROWS and -1 < currentCol and currentCol < BOARD_COLS and
...
# -----------restart fn after pressing button r-------------
def restart():
    screen.fill(BG_COLOR)
    draw_lines()
    for row in range(BOARD_ROWS):
...
# --------best move calculation for computer-----------------------
def bestMove(player=2):
    ...
    # ----------------------------computer first initialization move------------------------------------------
    if (currentRow == -1 or currentCol == -1):
        for row in range(BOARD_ROWS):
            for col in range(BOARD_COLS):
                if (board[row][col] == 0):
                    ...
                    if (score > bestScore):
                        bestScore = score
                        move = (row, col)
    # ------------------after first move of computer checking its adjacent valid cells------------------------------------------------------------
    if (-1 < currentRow - 1 and currentRow - 1 < BOARD_ROWS and -1 < currentCol - 1 and currentCol - 1 < BOARD_COLS and
    ...
    if (-1 < currentRow - 1 and currentRow - 1 < BOARD_ROWS and -1 < currentCol and currentCol < BOARD_COLS and
    ...
    if (-1 < currentRow and currentRow < BOARD_ROWS and -1 < currentCol + 1 and currentCol + 1 < BOARD_COLS and
            board[currentRow][currentCol + 1] == 0):
...
# ------------score fn values------------
scores = {
    1: 10,  # -------computer win state,player 1(human) lose------------
    2: -10,  # -------computer lose state----------
    0: 0
}
# --------------dictionary to track boards situation,minmiax starts--------------------
dict = {}


def minimax(board, player, playerOneCurrentRow, playerOneCurrentCol, playerTwoCurrentRow, playerTwoCurrentCol, depth,
            isMaximizing):
    ...
    # --------------computer (max) move, computer 2,ismaximizing true-----------------------------------------------------
    if isMaximizing:
        bestScore = -100000
        ...
        if (
            ...
            # print('IF:0')
            board[currentRow - 1][currentCol - 1] = 2
            score = minimax(board, 1, playerOneCurrentRow, playerOneCurrentCol, currentRow - 1, currentCol - 1, 0,
                            False)
            board[currentRow - 1][currentCol - 1] = 0
        ...
        return bestScore
    # -----------------------human(min) move,human 1,ismaximizing false------------------------------------------------------------------
    else:
        bestScore = 100000
...
# -------------randomly block some the squares when the game starts--------------
random_block_list = []
...
def start():
    ...
    text_rect3 = img3.get_rect(center=(300, 550))
    ...
    # button_6_5_img = pygame.image.load('button_6_5.png').convert_alpha()
    ...
    # button_5_5 = Button(500, 300, button_5_5_img)
    ...
    if (howTOplay):
        ...
        board = np.zeros((BOARD_ROWS, BOARD_COLS))
        draw_lines()
        # ------------------random block squares when board is initialized-----------
        random_block_square()
...
# -----------suggest human player possible moves-----------------------------------
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and not game_over and howTOplay == False and (
                block == -1 or block == 1):
            ...
            if available_square(clicked_row, clicked_col, 1):
                ...
                if check_lose(playerTwoCurrentRow, playerTwoCurrentCol):
                    ...
                    # ---------if not lost, give the best move -------------------------------------------
                    player = 2
                    bestMove(player)
                    draw_figures()
                    ...
                    # ---------after computer's best move ,check if human player lost the game -------------------------------------------
                    if check_lose(playerOneCurrentRow, playerOneCurrentCol):
                        ...
                        # ---------if human player didnot lose the game human will give block move to isolate/trap computer------------------------------------
                        if block == -1:
                            block = 1
                            help_text_draw(1)
                        else:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3 and not game_over and block == 2:
            ...
            # ---------if human didnt lose and chose a square to block then mark that sqaure ------------------------------------
            ...
            if board[clicked_row][clicked_col] == 0 and not game_over:
                ...
                help_text_draw(4)
                pygame.time.delay(5000)
            # ---------check if computer has lost(no valid move left) after human's block move ------------------------------
            if check_lose(playerTwoCurrentRow, playerTwoCurrentCol):
                losePlayer = 2
                game_over = True
                help_text_draw(5)
                ...
                # --------- if computer has not lost, computer will give block move ------------------------------
                print("Hello")
                # ---------computer will check human's available squares to move then create its own blockable list ------------------------------
                ...
                if len(blockable_square_list) != 0:
                    ...
                    mark_square(x, y, 2)
                # ---------after block check if human has lost ------------------------------
                if check_lose(playerOneCurrentRow, playerOneCurrentCol):
